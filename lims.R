...
lims.log <- function(prefix='        ',scriptName='defaultScriptName',msg=msg,file=NULL) {
	...
	message <- paste(date(),prefix,scriptName,': ',msg,'<br>',sep='')
	print(message)
...
lims.setMask <- function(spectra,list=list()) {
...
# lims.setMask(spectra,list=c(8.4,8.7,8.8,9,9.5,9.57))
# plot(spectra)

# fake <- list(x=c(1:100),y=sin(seq(1,6.27,length.out=100)))
# s <- lims.spectra(fake)
# plot(s)
# lims.setMask(s,list=c(10,20,40,50,60,100))
# plot(s)
...
lims.getJSON<-function(url,LOG=FALSE) {
	...
	if ( is.vector(url) & !is.list(url) ){
		if (length(url) == 0) {
			...
			if (LOG) {
				msg <- paste("<b>*** started with ",url,"</b>")
				log(prefix='        ',scriptName='scriptName',msg=msg,file=file)
				msg <- paste("<b>*** finished dim for json_data",length(json_data$entry[[1]]),"</b>")
				log(prefix='        ',scriptName='scriptName',msg=msg,file=file)
			}
			return(json_data)	
		}
...
lims.getNmrs <- function(entryList=entryList,entryData=entryData,Filter=list(),OP='AND',dry=FALSE) {
		...
		for (j in 1:length(x$nmrs)) {
			...
			if (eval(parse(text=z))) {
				...
				if (!dry) {
					...
					lims.log(prefix='        ',scriptName=functionName,msg=msg,file=logfile)
				}
				# merge infos
...
lims.createDataSet <- function(nmrList) {
	...
	return(list('nmrData'=nmrData,'ppm'=ppm,'param'=nmrList$param))
}
# example
...
#Filter <- list('experiment'=c('zg30','noesygpps1dcomp'),'solvent'=c('C6D6','COFFEEmeoh'))
Filter <- list('experiment'=c('noesygpps1dcomp'),'solvent'=c('COFFEEmeoh'))
nmrList <- lims.getNmrs(entryList,entryData,Filter=Filter,OP='AND',dry=TRUE)
